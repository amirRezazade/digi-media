      ...
      if (elem.poster_path && elem.backdrop_path) {
        ...
        document.querySelector("#header-swiper-wrapper").innerHTML += `
          <a 
                        ...
                        class="swiper-slide rounded-md group flex  duration-500 min-h-[300px] sm:min-h-[235px] md:min-h-[246px] lg:min-h-[235px] xl:min-h-[230px] 2xl:min-h-[260px] "  data-index="${index}" data-id="${elem.id}"
                      >
                        <div
                        ...
                        >
                          ...
                          <div
                          dir="ltr"
                            class="text-center h-5 text-sm text-white truncate group-hover:text-orange-400 transition-colors duration-600"
                          >
...
async function getActionMovie() {
  ...
  addCarts(randomFive, document.querySelector(".action-movie"));
  generateSwiper(document.querySelector(".movieSwiper"));
}
async function getTv() {
  let response = await fetch(`https://api.themoviedb.org/3/discover/tv?api_key=${apiKey}&with_genres=18&without_genres=16,10751&sort_by=vote_average.desc&vote_count.gte=1000`);
  let t = await response.json();
  let list = t.results;
  const randomFive = list.filter((item) => item.poster_path && item.backdrop_path).slice(0, 6);
...
async function getPersianMovie() {
  let response = await fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${apiKey}&with_original_language=fa&with_production_countries=IR&sort_by=vote_average.desc`);
  let t = await response.json();
  let list = t.results;
  const randomFive = list.filter((item) => item.poster_path && item.backdrop_path).slice(0, 6);

  addCarts(randomFive, document.querySelector(".persian-movie"));
  ...
  list.forEach((elem) => {
    if (elem.poster_path && elem.backdrop_path) {
      wrapper.innerHTML += `
             ...
             <p dir="ltr" class="block mt-2  h-auto w-full text-center truncate  text-ellipsis transition-colors duration-300 group-hover:text-orange-400 text-black dark:text-white text-sm "><span class="">${elem.name ? elem.name : elem.title}</span><span class="opacity-70 ml-2 ">${elem.release_date ? elem.release_date.slice(0, 4) : ""}</span></p>
            </a>
            `;
    }
  });
