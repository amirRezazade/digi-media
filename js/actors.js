    ...
    if (data.birthday || data.imdb_id) {
      document.querySelector("#card").innerHTML = `
       <div class="w-full xs:min-w-20 xs:max-w-50 max-h-75 sm:max-w-80 sm:max-h-80 sm:w-full overflow-hidden">
        <img loading="lazy" class="aspect-square object-cover h-full sm:h-[330px] w-full loading-animation" src="https://image.tmdb.org/t/p/original${data.profile_path}_medium" alt="${data.name}" onerror="this.onerror=null; this.src='images/default_poster.jpg';">
       </div>
      ...
      `;
      document.querySelector("#card img").addEventListener("load", () => document.querySelector("#card img").classList.remove("loading-animation"));
    } else {
  ...
  validItems.slice(page * itemInPage - itemInPage, page * itemInPage).forEach((elem) => {
    ...
    <a href="${elem.media_type == "tv" ? "series" : "movie"}.html?id=${elem.id}" class=" w-auto  rounded-2xl overflow-hidden transition-all duration-600 group">
          ...
          <div class="w-full h-full relative rounded-lg overflow-hidden">
            <img loading="lazy" class="object-cover w-full h-full loading-animation" src="https://image.tmdb.org/t/p/original/${elem.poster_path}_medium" alt=""  onerror="this.onerror=null; this.src='images/default_poster.jpg';">
       ...
       `;
    document.querySelectorAll("#items img").forEach((elem) => elem.addEventListener("load", () => elem.classList.remove("loading-animation")));
  });
  paginationControl();
}
