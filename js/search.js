...
function addItems(list) {
  if (list.length == 0) {
    ...
    list.forEach((elem) => {
      ...
      let type = elem.release_date ? "movie" : "tv";
      let hrefType = elem.release_date ? "movie" : "series";
      let mediaType;
      if (elem.media_type == "tv") mediaType = "series";
      if (elem.media_type == "movie") mediaType = "movie";
      if (elem.media_type == "person") mediaType = "actors";
      document.querySelector("#items").innerHTML += `
       <a
               ...
               <div class="absolute top-0 left-0 flex flex-col justify-between  w-full min-h-full bg-black/60 px-3 py-3.5">
                   <div class=" flex justify-between items-center">
                     ...
                     <span class=" font-light text-sm text-amber-400">${date ? date.slice(0, 4) : "..."}</span>
                       <span class=" font-light text-sm text-amber-400">${elem.media_type ? elem.media_type : type}</span>
                   </div>
                  ...
                  <svg width="40px" height="40px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                   ...
                   <div class="flex text-gray-300 flex-col items-start gap-3">
                    ...
                    <span class="flex items-center gap-0.5"><span class="text-xs ${elem.vote_average ? "" : "hidden"}">10/</span>
                      <span class="text-lg md:text-xl xl:text-1xl text-amber-400 font-bold">${elem.vote_average ? elem.vote_average.toFixed(1) : ""}</span></span>
                  </div>
                   </div>
               </div>
           ...
           <p dir="ltr" class="mt-2 lg:mt-0 h-auto w-full text-center truncate text-ellipsis transition-all duration-300 group-hover:text-amber-400 text-black dark:text-white text-sm ">${elem.title ? elem.title : elem.name}</p>
          </a>
          `;
    });
    document.querySelectorAll("#items img").forEach((img) => img.addEventListener("load", () => img.classList.remove("loading-animation")));
  }
  paginationControl();
}
