...
function search(){
  document.querySelector('#items').innerHTML=''
    ...
    let country =document.querySelector('#country').value=='' ? null : `&with_origin_country=${document.querySelector('#country').value}`
    let genre =document.querySelector('#genre').value=='' ? null : `&with_genres=${document.querySelector('#genre').value}`
    let fromYear =Number(document.querySelector('#fromYear').value)
    let toYear =Number(document.querySelector('#toYear').value)
    let fromPoint =document.querySelector('#fromPoint').value==0 ? null : `&vote_average.gte=${Number(document.querySelector('#fromPoint').value)}`
    let toPoint =document.querySelector('#toPoint').value==0 ? null : `&vote_average.lte=${Number(document.querySelector('#toPoint').value)}`
    let age =document.querySelector('#age').value=='' ? null : `&certification=${document.querySelector('#age').value}`    
...
async function movie(country , genre , fromYear , toYear , fromPoint , toPoint , age ){
 ...
 let response = await fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${apiKey}${genre ? genre : ''}${country ? country : ''}${fromPoint ? fromPoint : ''}${toPoint ? toPoint : ''}${fromYear==0 ? '' : '&primary_release_date.gte='+fromYear+'-01-01'}${toYear==0 ? '' : '&primary_release_date.lte='+toYear+'-01-01'}${age ? age : ''}`)
...
async function tv(country , genre , fromYear , toYear , fromPoint , toPoint , age ){
  ...
  let response = await fetch(`https://api.themoviedb.org/3/discover/tv?api_key=${apiKey}${genre ? genre : ''}${country ? country : ''}${fromPoint ? fromPoint : ''}${toPoint ? toPoint : ''}${fromYear==0 ? '' : '&first_air_date.gte='+fromYear+'-01-01'}${toYear==0 ? '' : '&first_air_date.lte='+toYear+'-01-01'}${age ? age : ''}`)
  let res = await response.json();
  let list = res.results;
  console.log(list);
  addItems(list)
...
async function all(country , genre , fromYear , toYear , fromPoint , toPoint , age ){
  let movieResponse = await fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${apiKey}${genre ? genre : ''}${country ? country : ''}${fromPoint ? fromPoint : ''}${toPoint ? toPoint : ''}${fromYear==0 ? '' : '&primary_release_date.gte='+fromYear+'-01-01'}${toYear==0 ? '' : '&primary_release_date.lte='+toYear+'-01-01'}${age ? age : ''}`)
 let movieRes = await movieResponse.json();
 let movieList = movieRes.results;
 let tvResponse = await fetch(`https://api.themoviedb.org/3/discover/tv?api_key=${apiKey}${genre ? genre : ''}${country ? country : ''}${fromPoint ? fromPoint : ''}${toPoint ? toPoint : ''}${fromYear==0 ? '' : '&first_air_date.gte='+fromYear+'-01-01'}${toYear==0 ? '' : '&first_air_date.lte='+toYear+'-01-01'}${age ? age : ''}`)
  ...
  list.forEach(elem=>{
    ...
    `
     <a
         ...
         href="${elem.first_air_date ? 'series' : 'movie'}.html?id=${elem.id}"
          id="${
           elem.id
         }"  class=" inline-flex flex-col items-center w-full  gap-1.5 overflow-hidden  rounded-xl transition-all duration-600 group">
